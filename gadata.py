# This is the modified version of gadata by Aza Jalalvand to make it compatible with python 3 (2025)
    ...
    def __init__(self,signal,shot,tree=None,connection=None,nomds=False):
        ...
        if nomds == False:
            try:     
                ...
                self.rank    = numpy.linalg.matrix_rank(self.zdata)
                ...
                found = 1
                source = f"MDSplus tree {fstree}"
                # # MDSplus seems to return 2-D arrays transposed.  Change them back.
                # if numpy.linalg.matrix_rank(self.zdata) == 2: self.zdata = numpy.transpose(self.zdata)
                # if numpy.linalg.matrix_rank(self.ydata) == 2: self.ydata = numpy.transpose(self.ydata)
                # if numpy.linalg.matrix_rank(self.xdata) == 2: self.xdata = numpy.transpose(self.xdata)
        ...
        if found == 0: 
            ...
            t1 = time.time() - t0
            print (f'   {signal} fetched from {source} in {t1:.2f}s')
            # print ('   GADATA Retrieval Time :',t1)
